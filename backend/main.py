...
def cleanup_db(db: Session = Depends(get_db), _: User = Depends(require_admin)):
    ...
    def _run():
        try:
            ...
            # Remove district parquet data for deleted sessions
            try:
                import shutil as _sh2
                _dd = os.path.join(DATA_DIR, "district_data")
                if os.path.isdir(_dd):
                    for _sd in os.listdir(_dd):
                        try:
                            if int(_sd) != active_id:
                                _sh2.rmtree(os.path.join(_dd, _sd), ignore_errors=True)
...
def export_district(
    ...
    # ── find parquet files for this district ─────────────────────────────────
    _pq_dir = os.path.join(DATA_DIR, "district_data", str(sid), district)
    _pq_files = sorted(_glob.glob(os.path.join(_pq_dir, "*.parquet")))
    if not _pq_files:
        raise HTTPException(
            404,
            "Данные для экспорта не найдены. Пожалуйста, переобработайте файлы через кнопку «Перезапустить обработку» в разделе Управление.",
        )
    ...
    with _pd.ExcelWriter(output, engine="openpyxl") as writer:
        df_out.to_excel(writer, index=False, sheet_name="Данные")
    output.seek(0)
